...
SOURCE FILE GUIDE: Quick routing map — what each file owns and when to read it.
  index.js — turn orchestration, narrator/RC/CB/ORS pipeline, all prompt assembly, gates, and intercepts | read when tracing a turn pipeline fault, prompt instruction, or gate behavior | NOTE: all /diagnostics/* routes extracted to diagnostics.js as of v1.88.55 | GAMEPLAY INGRESS: POST /narrate is the primary gameplay endpoint — handles Turn 1 (founding premise, no x-session-id header, action field contains world premise) and all subsequent turns (x-session-id header required, action field is player input); returns { sessionId, narrative, ... }
  diagnostics.js — backend diagnostics subsystem: all /diagnostics/* route handlers, diagnostics state (_lastGameState, _lastSessionId, _diagHistory etc.), SSE infrastructure (emitDiagnostics, registerStreamHandler), buildDebugContext | read when tracing a /diagnostics/* route fault, auth behavior, or response shape | NOT the SSE terminal client — that is flight-recorder.js | AUTH GATE PATTERNS: two distinct guards exist in this file — (1) source + source-search return 401 + {"error":"unauthorized"} when no key is present; (2) npcs + mb-crash return 403 + {"error":"forbidden"} when no key is present. Both patterns correctly block unauthenticated access. The 401/403 split is a known inconsistency, not a security gap. | ROUTE INVENTORY (24 routes) — no-session/no-auth: GET /session, GET /summary, GET /site-placement, GET /stream (SSE), GET /npc (singular — current session NPC state, no auth required), GET /sites; keyed-401 (x-diagnostics-key, returns unauthorized): GET /source, GET /source-search; keyed-403 (x-diagnostics-key, returns forbidden): GET /npcs (plural — full NPC listing, distinct from /npc), POST /mb-crash; turn/session: GET /turn/:sessionId/:turn, GET /turn/latest, GET /payload/:sessionId/:turn, GET /log (turn history), GET /context; objects/entity: GET /objects, GET /entity, GET /objects/trace, GET /continuity; world/sites: GET /sites-query, GET /site (singular), GET /localspaces (plural), GET /localspace (singular); mutation: POST /inject-npc (keyed).
