...
- TAKE whole-object: ActionProcessor executes synchronously and calls transferObjectDirect directly. Pushes the object ID to gameState._apExecutedTransfers[].
...
Do not infer execution ownership from a reason string's 'tls_' prefix -- whole-object TAKE is AP-executed but still carries the reason 'tls_whole_object_take'. Determine ownership from which receipt is populated (_apExecutedTransfers vs _tlsExecutionResult vs _tlsPartialStackResult) and from gameState._apActuals.routing (quarantined or not present), not from string naming alone.
...
SEMANTIC PARSER INTERNALS (SemanticParser.js — implementation detail for investigation use): normalizeUserIntent(input, apiKey, channel, gameState) is the entry point. Fast-path regexes fire first (no LLM, confidence 0.97) for: grab/snatch patterns (→ take), bare-entry inputs (e.g. "go in" → enter), aggregate-remove patterns, single-item-remove patterns — debug shows FAST_PATH_HIT when this path fires. 30-second TTL cache keyed on input hash (SHA256 of lowercased+trimmed input): if an identical input is seen within 30 seconds, the cached result is returned and LLM is skipped. LLM path: DeepSeek-chat, temperature 0.0, max_tokens 256; channel ('do' vs 'say') affects prompt construction — 'do' channel enables action parsing logic, 'say' channel biases toward NPC targeting and dialogue intent. resolveEnterTarget() is a second independent LLM call, fired only when the parsed intent lacks both a target and a direction — it is NOT part of the main normalizeUserIntent pipeline and should be investigated separately if entry-target resolution fails. narration_debug.semantic_parser shows FAST_PATH_HIT or LLM_PARSE and the reason; when fast path fires, no DS call appears in stage_times or payload archive. SemanticParser owns the parsed_action and target fields only — it never writes game state.
