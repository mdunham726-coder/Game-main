...
ObjectRecord schema: id (deterministic — see below), name, description, status (active|transferred|archived), current_container_type (player|npc|grid|localspace|site), current_container_id, created_turn, conditions[] (physical state history — each entry: {description, set_turn, evidence}; see below), events[] (array of lifecycle events — each: action, from_container_type, from_container_id, to_container_type, to_container_id, turn, timestamp). IMPORTANT: the actual container_type value for cell/grid containers is 'grid' (not 'cell'). Container types in code: player, npc, grid, localspace, site. Do not confuse 'grid' with 'cell' — any ObjectRecord showing current_container_type:'grid' is held by a world cell, identified by current_container_id (format: LOC:mx,my:lx,ly). 'localspace' = L2 interior floor (container_id = local_space_id). 'site' = L1 site floor tile (container_id = \${site_id}:\${x},\${y}).
...
Container model: every object has exactly one container at any time. Container types: player (gameState.player.object_ids[]), npc (an NPC record's object_ids[]), grid (a world cell's object_ids[], container_type value is 'grid'), localspace (an L2 interior floor — _generated_interior.object_ids[], container_id = local_space_id), site (an L1 site floor tile — active_site.floor_positions[x,y].object_ids[], container_id = \${site_id}:\${x},\${y}). Transfer updates current_container_type, current_container_id, and adds a transfer event. One-container enforcement: if an object appears in two containers, the system emits an error and resolves to the most recent assignment. object_ids[] on the container side is authoritative; current_container_* on the ObjectRecord mirrors it.
