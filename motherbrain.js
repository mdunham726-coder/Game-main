...
ObjectRecord schema: id (deterministic — see below), name, description, status (active|transferred|archived), current_container_type (player|npc|grid), current_container_id, created_turn, conditions[] (physical state history — each entry: {description, set_turn, evidence}; see below), events[] (array of lifecycle events — each: action, from_container_type, from_container_id, to_container_type, to_container_id, turn, timestamp). IMPORTANT: the actual container_type value for cell/grid containers is 'grid' (not 'cell'). Container types in code: player, npc, grid. Do not confuse 'grid' with 'cell' — any ObjectRecord showing current_container_type:'grid' is held by a world cell, identified by current_container_id (format: LOC:mx,my:lx,ly).
...
object_reality frozen turn field shape: { ran: bool, promoted: int, transferred: int, errors: int, pre_rejected: int, skip_reason: string|null, error_entries: [{action, object_name, object_id, reason}], audit: [{object_id, action, from_container_type, from_container_id, to_container_type, to_container_id}], condition_updates: [{applied: bool, objectId, reason}], retirement_updates: [{retired: bool, objectId, reason}] }. condition_updates and retirement_updates are always present (empty array if no updates/retirements this turn). pre_rejected is the count of grid promote entries rejected by the pre-flight normalization gate before ObjectHelper ran. When ran:false, skip_reason is populated (e.g. empty_quarantine, no_phaseB_result). When ran:true, promoted and transferred are the counts for that turn. Errors are non-fatal — the pipeline continues; error_entries give the per-error detail. Use trace_object to see all audit entries for a specific object across turns.

Container model: every object has exactly one container at any time. Container types: player (gameState.player.object_ids[]), npc (an NPC record's object_ids[]), grid (a world cell's object_ids[], container_type value is 'grid'). Transfer updates current_container_type, current_container_id, and adds a transfer event. One-container enforcement: if an object appears in two containers, the system emits an error and resolves to the most recent assignment. object_ids[] on the container side is authoritative; current_container_* on the ObjectRecord mirrors it.
...
STATE CLAIM ROUTING: state_claim is a parser routing verdict, not an engine action. It signals that the player input was a bare assertion (possession, existence, identity, condition, or world fact) with no concrete mechanical intent. When parsed_action is state_claim, the engine intercepts before validation, routes to the freeform channel (debug.path: STATE_CLAIM_FREEFORM), and skips the Reality Check (skipped_reason: state_claim). The narrator instruction depends on whether the player has established founding attributes (declared:, physical:, or object: bucket entries in player.attributes): (A) If established attributes exist — the narrator receives those attributes explicitly and an escape hatch: if the attempted action is a direct or natural expression of any of those attributes, treat it as a real attempt with real consequences; the denial block still applies for anything not supported by those attributes. (B) If no established attributes — blanket denial: claim is unsupported, do not instantiate objects/inventory/conditions/NPCs/authority/world facts, reflect only existing engine state, narrate denial explicitly. Degraded state_claim paths (TARGET_NOT_FOUND_IN_CELL, PARSER_FAILURE_FALLBACK) always receive the blanket denial — they are parser/navigation failures, not ability assertions. This routing is correct behavior, not a fault. Do not flag debug.path: STATE_CLAIM_FREEFORM or skipped_reason: state_claim as anomalies.
