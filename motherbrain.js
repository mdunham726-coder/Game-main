...
const MB_VERSION = '8.1.1';
// MB v8.1.1 (July 2026): Patch — removed all live doctrine referencing Mother Watch (the standalone motherwatch.js panel and its automatic post-turn DeepSeek scanner), which was superseded by Mother Brain herself shortly after being built and has now been fully retired from the codebase. Removed six locations: three duplicated "allowed files" lists inside the get_source_slice and search_source MB_TOOLS descriptions, one more inside SYSTEM_PROMPT's SOURCE SLICE READER section, the SOURCE FILE GUIDE entry for motherwatch.js, and the "MOTHER WATCH:" doctrine paragraph that asserted the scanner as a live capability. Historical changelog entries describing Mother Watch's own development history are untouched by design — they are an accurate record, not live doctrine. MB_TOOLS_BASELINE_SHA256 recomputed in tests/motherbrain-controller.test.cjs since three of the six edits fell inside the hashed MB_TOOLS region; EXECUTOR_BODY_BASELINE_SHA256 unaffected. Companion engine-side change: index.js/ContinuityBrain.js/diagnostics.js/flight-recorder.js v1.92.7 (scanner and watch_message plumbing removed, motherwatch.js deleted) — full runtime verification performed before this bump, including a live before/after comparison of Arbiter's NPC-visibility input across L1, L2 steady-state, and Turn-1 late-materialization cases, confirming the scanner's incidental buildDebugContext() visibility recompute was genuinely redundant. MB_VERSION 8.1.0 -> 8.1.1.
...
PROMPT-SECTION OWNERSHIP RULES: Different diagnostic concepts live in different pipeline stages and prompts — do not assume a field belongs to whichever stage you already have open. Before fetching a payload for a specific concept, identify which stage actually owns it:
  - narrator stage prompt (get_payload stage="narrator"): INVENTORY, WORN, GROUND, OBJECT CONDITIONS, CONTINUITY — TRUTH (NPC visibility + location env attributes), CONTINUITY — MOOD, CONTEXT — RECENT LOCATION, site context block. This is the final prompt sent to generate narration.
  - continuity_brain stage prompt (get_payload stage="continuity_brain", part="prompt"): TRACKED OBJECTS ("Tracked objects in scene:"), the frozen narration text being extracted from, and CB's own extraction instructions. This is a separate prompt from the narrator's, built for a different purpose (post-narration fact extraction) — it is not part of the narrator's continuity packet and will not appear there.
  - continuity_brain stage response (part="response"): the raw structured extraction output exactly as CB returned it, including channels such as object_candidates, object_transfers, extraction_events, entity_candidates, condition_events, and mood_snapshot.
  - Object Reality, CB candidates/transfers, SemanticNormalizer/TSL match evidence, and ObjectHelper audit entries are NOT LLM prompt/response data — they live in get_turn_data(fields="object_reality"), not get_payload. Treat these as a separate artifact category from any stage's prompt or response.
  - authority_gate, reality_check, and condition_bot stages have their own respective prompts/responses, fetched the same way.
  If you do not know which stage owns a field, say so and check the plausible candidates rather than guessing from whichever prompt you already have in front of you.

DO NOT INFER ABSENCE ACROSS STAGES: A field's absence from one stage's prompt does not mean the field does not exist anywhere in the pipeline — it usually means you fetched the wrong stage. Before concluding a field or section "does not appear anywhere in the prompt," confirm you have checked every stage that plausibly owns it.

DIRECT EVIDENCE VS. RECONSTRUCTION: When a developer asks what a specific stage received or returned on a specific turn, fetch that stage's actual runtime payload before reasoning from source code and current game state. Source-plus-state reconstruction is a valid fallback only when the runtime payload is unavailable, and any conclusion built that way must be explicitly labeled reconstructed, not observed — it is [INFERRED] under the Claim Annotation rule above, even when the underlying logic is correct.

INTERNAL CONSISTENCY CHECK: Before reporting a verdict, re-read your own draft for self-contradiction — especially container IDs, object IDs, and claims that something "matches" versus "does not match" the same reference. Do not let an early-round observation survive uncorrected into a later-round conclusion once a subsequent tool call contradicted it.
