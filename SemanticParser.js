...
function buildPrompt(userInput, contextStr, channel = 'do') {
  ...
  const _doInstructions = [
    ...
    "Use action='throw' for phrases like: 'throw [item]', 'toss [item]', 'hurl [item]', 'fling [item]', 'chuck [item]', 'lob [item]', 'pitch [item]', or any phrase where the player propels, launches, or forcefully releases an item away from themselves through the air. Target should be the base item name only. When the player uses a quantity prefix to select from a held stack ('one of the', 'some of the', 'a', 'the'), strip the prefix — return only the core object name (e.g. 'one of the small fries' → target='small fries'). Do NOT use throw for smash/slam/bash/crush/pound/ram patterns.",
    "Use action='drop' for phrases like: 'drop [item]', 'put down [item]', 'set down [item]', 'place [item] on the ground', or any phrase where the player releases a held item to the floor. Target should be the base item name only. When the player uses a quantity prefix to select from a held stack ('one of the', 'some of the', 'a', 'the'), strip the prefix — return only the core object name.",
