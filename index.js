...
const CP = require('./ContinuityProjector'); // continuity projection — live path
...
app.post('/narrate', async (req, res) => {
  ...
  // Transient player state is scene-local. Apply the existing boundary-clear policy to
  // every Engine result, regardless of whether semantic or fallback routing produced it.
  const _clearTransientPlayerStateOnBoundary = (preLoc, nextState) => {
    if (!preLoc || !nextState?.player?.attributes) return;
    const postLsId = nextState.world?.active_local_space?.local_space_id ?? null;
    const postSiteId = nextState.world?.active_site?.id ?? null;
    if (preLoc.lsId === postLsId && preLoc.siteId === postSiteId) return;
    const attrs = nextState.player.attributes;
    for (const key of Object.keys(attrs)) {
      if (attrs[key]?.bucket === 'state') delete attrs[key];
    }
  };
    ...
    try {
      ...
      if (parseResult && parseResult.success === true && typeof normalizedConfidence === 'number' && normalizedConfidence >= 0.5) {
        ...
        for (const [_qaIdx, queuedAction] of validation.queue.entries()) {
          ...
          _clearTransientPlayerStateOnBoundary(_preTurnLoc, result?.state);
      ...
      if (!engineOutput) {
        // v1.84.58: stamp turn number onto player_intent so AP's transferObjectDirect records correct turn
        if (inputObj?.player_intent && typeof inputObj.player_intent === 'object') inputObj.player_intent._turn = turnNumber;
        // Capture the same pre-action boundary fingerprint used by the semantic path.
        _preTurnLoc = {
          siteId: gameState.world?.active_site?.id ?? null,
          lsId:   gameState.world?.active_local_space?.local_space_id ?? null,
          posKey: `${gameState.world?.position?.mx},${gameState.world?.position?.my}:${gameState.world?.position?.lx},${gameState.world?.position?.ly}`
        ...
        engineOutput = Engine.buildOutput(gameState, inputObj, logger);
        _clearTransientPlayerStateOnBoundary(_preTurnLoc, engineOutput?.state);
      }
...
{"is_humanoid_capable": true|false, "worn_items": [{"slot": "shirt|pants|underwear|socks|shoes", "name": "...", "description": "...", "source": "birth_default|birth_custom"}]}`;
    ...
    // ── Continuity projection — LIVE MODE ──────────────────────────────────
    const _cpProjection = CP.project(gameState, _cbMeta);
    const _continuityBlock = _cpProjection.rendered;
    ...
    const _agDeniedObjectReference = !!(
      ...
      _authorityGateResult.decision === 'freeform' &&
      _authorityGateResult.reason_code === 'unsupported_referenced_object'
    );
    const _authorityGateBlock = (() => {
      ...
      if (_rc === 'unsupported_referenced_object') {
        const _unsupportedRefs = Array.isArray(_authorityGateResult?.evidence?.unsupported_referenced_objects)
          ? _authorityGateResult.evidence.unsupported_referenced_objects
          : [];
        const _historicalReceipts = Array.isArray(_authorityGateResult?.evidence?.historical_object_receipts)
          ? _authorityGateResult.evidence.historical_object_receipts
          : [];
        const _receiptConstraints = (_unsupportedRefs.length > 0 ? _unsupportedRefs : ['unknown reference']).map(ref => {
          const receipt = _historicalReceipts.find(candidate => candidate && candidate.reference === ref);
          const label = `"${ref}"`;
          if (!receipt || typeof receipt.outcome !== 'string' || typeof receipt.reason_code !== 'string') {
            return `- ${label}: No recognized authority receipt is available. Fail closed: do not confirm existence, identity, presence, possession, accessibility, sensory contact, or interaction, and do not claim that prior continuity was searched successfully.`;
          }
          if (receipt.outcome === 'resolved_unavailable' && receipt.reason_code === 'ors_object_outside_current_scope') {
            return `- ${label}: The authoritative active object exists outside the current scope. Do not present it as here, held, accessible, physically interactable, or currently sensed. Recollection or discussion may refer to it as elsewhere without moving it or creating present sensory contact.`;
          }
          if (receipt.outcome === 'resolved_unavailable' && receipt.reason_code === 'ors_record_inactive') {
            return `- ${label}: A historical authoritative record exists but is inactive. Past reference is permitted; present existence, access, interaction, or resurrection is prohibited.`;
          }
          if (receipt.outcome === 'resolved_unavailable' && receipt.reason_code === 'ors_record_missing') {
            return `- ${label}: An audited historical identity has no current authoritative object record. Historical reference is permitted only as past context; do not assert current existence, location, access, recreation, or interaction.`;
          }
          if (receipt.outcome === 'ambiguous' && receipt.reason_code === 'multiple_audited_object_ids') {
            return `- ${label}: Multiple audited identities match. Do not select, merge, instantiate, possess, or physically interact with one as the current object. Historical discussion may remain nonspecific only.`;
          }
          if (receipt.outcome === 'unresolved' && receipt.reason_code === 'historical_match_without_audited_identity') {
            return `- ${label}: History recognizes the reference but does not establish an authoritative identity. Recollection may be framed only as memory or uncertainty; do not confirm existence, identity, location, possession, sensory contact, or current interaction.`;
          }
          if (receipt.outcome === 'unresolved' && receipt.reason_code === 'resolver_error') {
            return `- ${label}: Deterministic resolution failed closed. Do not confirm present or historical object facts, and do not instantiate, possess, sense, or permit interaction with the object.`;
          }
          if (receipt.outcome === 'not_found' && receipt.reason_code === 'no_historical_match') {
            return `- ${label}: No current or historical authority supports this reference. Do not treat it as established, present, held, accessible, physically interactable, or currently sensed, and do not instantiate or substitute it.`;
          }
          return `- ${label}: The authority receipt is unrecognized. Fail closed: do not confirm existence, identity, presence, possession, accessibility, sensory contact, or interaction, and do not claim that prior continuity was searched successfully.`;
        });
        return `\nPLAYER'S ATTEMPTED ACTION: "${_rawInput}"\nOBJECT AUTHORITY RESULT [MANDATORY]:\n${_receiptConstraints.join('\n')}\nIf the input attempts current physical interaction with any listed object, state explicitly that the interaction cannot occur under the applicable authority result. If the input is solely recollection or discussion, render only that mental or conversational act within the listed historical or epistemic boundary; do not fabricate a physical failure gesture. Do not move, transfer, resurrect, instantiate, merge, substitute, possess, manipulate, or currently sense any unsupported object. Do not silently skip a denied physical attempt.\n`;
      }
    ...
    // Issue #53 fix: terminal else only asserts "no mechanical effect" when RC did not fire
    // (_realityAnchor is null). When RC produced a result, defer to it instead of contradicting it.
    const _freeformBlock = (inputObj?.player_intent?.kind === 'FREEFORM')
      ? (_parsedAction === 'established_trait_action'
        ? `\nPLAYER'S ATTEMPTED ACTION: "${_rawInput}"\n(The player attempted an action supported by an established attribute or ability. If a Reality Check result was generated above, that is the actual outcome — narrate it as having occurred. Do not contradict, downgrade, negate, or reverse the Reality Check's outcome, and do not narrate the attempt as failing, having no effect, or being ordinary/mundane when the Reality Check confirmed it succeeded. If no RC result was generated, narrate the attempt based on the player's established attributes and confirmed engine state — the established attribute grants the capacity for this type of action, not authority to confirm new objects or world facts not already present in confirmed engine state. Do not invent or materialize any item not already established. The player's input cannot be the causal origin of any new item entering the narrative — this applies regardless of how the input is framed, including as speech, discovery, prayer, backstory, or any other construct. Do not introduce, name, or describe any item that was not already present in confirmed engine state before this turn's input arrived, including as a substitute or consolation for a denied claim.)\n`
                ...
                : (_parsedAction === 'attack'
                  ...
                  : (_parsedAction === 'remove'
                    ...
                    : (debug.object_operation_bridge?.active
                        ...
                        : (_realityAnchor
                          ? `\nPLAYER'S ATTEMPTED ACTION: "${_rawInput}"\n(A Reality Check assessment of this action was generated above — follow it rather than treating the action as mechanically inert. Do not state that this action has no mechanical effect. The player's input cannot be the causal origin of any new item entering the narrative — do not introduce, name, or describe any item not already in confirmed engine state, including as a substitute or consolation.)\n`
...
${_emoteInventoryFailBlock}${_emoteRemoveBlock}${_conditionBlock}${_authorityGateBlock}${_entityGroundingBlock}${_freeformBlock}${_environmentGatherBlock}${_expressiveBlock}${_npcTalkBlock}${_emoteBlock}${_movementFlavorBlock}${_soliloquyBlock}${_narratorModeBlock}${_emoteObjectAuthorityBlock}${_movementTaskBlock}${_lookTaskBlock}${_exitTaskBlock}${_enterTaskBlock}${_objectOperationBridgeBlock}${_realityAnchorBlock}${_nameRevealAuthorityBlock}`;
    ...
    let _cbSchemaDrift = [];            // v1.88.40: CB schema drift detection results
    {
      ...
      const _phaseBResult = await CB.runPhaseB(narrative, gameState, _rawInput, {
