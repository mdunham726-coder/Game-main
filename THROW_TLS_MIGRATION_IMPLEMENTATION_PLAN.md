...
- **Status:** MIGRATION COMPLETE (2026-07-22). All 13 stages plus Slice D1 done and validated (see companion roadmap for full per-stage evidence). THROW object-operation authority now runs entirely through TLS → ObjectHelper → ORS; the legacy AP THROW implementation has been removed (`ActionProcessor.js:675-765`, 91 dead lines), leaving AP's THROW handling structurally identical to its DROP quarantine stub. Stage 9 closed as an explicit split verdict on O8/issue #42, not an unqualified pass; Stage 12/13 carry forward four pre-existing, already-decided exceptions, none new and none regressions: V7/issue #40 ("all" quantity unreachable), V17 whole-turn/issue #42 (CB duplicate promotion), V21 live-labeling/issue #41 (diagnostics-only mislabel), V27/O7 (compound-turn CB-promotion risk, probabilistic). V6/V9/V10 remain source-review-validated per established precedent. No new TAKE or DROP regression attributable to this migration was observed (issue #38, unrelated and pre-existing, still exists independently).
...
- **O8 (accepted limitation, state-mutation class, confirmed distinct mechanism from O7 — deferred to the future mechanical-verbs project, not a plan gap):** whole-stack THROW's turn-level zero-duplication requirement (V17) fails on a plain single-action turn, not only on compound queues. Live-confirmed at Stage 9: a 5-unit `coins` stack thrown via `"throw 5 coins"` moved correctly and intact through the TLS → ObjectHelper authority chain (proving the exact-count/full-stack routing question GPT raised before Stage 9 could be called closed), but Continuity Brain then independently promoted three additional objects (summing to the same quantity) from its own reading of the turn's narration, defeating the generic exact-name dedup guard on a plural/singular mismatch (`"coins"` vs `"coin"`). Distinct from O7: O7's gap is that the seal never activates for compound queues (an acknowledged, by-design scope exclusion); O8's gap is that the seal activated and worked correctly through execution, then correctly deactivated after a real transfer — and nothing distinguishes CB's ordinary post-execution narrative extraction from CB re-describing the object(s) just mechanically handled on that same turn. A future fix must suppress CB's promotion of the specific identities/quantities mechanically handled on a turn, not merely gate CB during the pre-execution/observe window as the current seal does. Filed as issue #42. Confirmed pre-existing and cross-verb (the same broad CB narrative-promotion pathway previously produced an extra promoted object after a plain TAKE); not introduced by this migration, and not implicated in the TLS/ObjectHelper/ORS authority chain the migration builds — none of that chain's diagnostic counters are touched by CB's independent promote path. Explicitly not expanding Stage 11 to contain this (Stage 11 remains scoped to Bug B only, per its original text and O2); deferred to the future mechanical-verbs/anti-FREEFORM project alongside O5 and O7, by direct user decision.
- **Approval blocker:** implementation unauthorized until user approval of this plan and the companion roadmap.

## Revision History

- **2026-07-22 — Revision 9 (Stage 13 complete — migration complete).** Legacy AP THROW implementation removed as dead code (91 lines, `ActionProcessor.js:675-765`); AP's THROW block now structurally identical to its DROP stub. Full-codebase grep confirmed nothing referenced the removed region. Validation gate (V1-proxy, partial-THROW smoke, V19-proxy, V23, V24) passes on the post-removal build, after one unrelated test-infrastructure hiccup (localspace-fill LLM parse failure, not a Stage 13 finding) was traced, excluded, and the affected rows retried with real passing evidence. Status changed to MIGRATION COMPLETE. See companion roadmap Revision 9 for full evidence. No architectural or contract change.
- **2026-07-22 — Revision 8 (Slice D1 landed; Stage 12 full validation matrix complete).** D1's operation-family guard implemented and live-verified (11/11 checks: DROP/THROW turns correctly diverted, TAKE unaffected and confirmed byte-equivalent via diff). Full V1-V27 matrix re-run/re-confirmed on the final build (21 live checks). A `missing_source` diagnostic anomaly surfaced during V7/V11 testing was investigated to source rather than accepted at face value, and confirmed as correct cascading fail-closed behavior (`_assembleTlsInstructionV1` withholds the source when the resolver already fail-closed; the executor's own `missing_source` check truthfully reports it) — not a new defect. Four rows (V7/O6, V17 whole-turn/O8, V21 live-labeling/issue #41, V27/O7) carry pre-existing exceptions already decided in earlier stages, re-confirmed unchanged, not re-litigated. See companion roadmap Revision 8 for the full per-row evidence. No architectural or contract change.
- **2026-07-22 — Revision 7 (Stages 10 + 11 complete as one deployment unit).** Partial-stack THROW live block + full Bug-B CB receipt chain and description reconciliation implemented per D6/D7, cloned end-to-end from DROP with THROW identity across `index.js` and `ContinuityBrain.js`. Live-verified V15 (6 partial throws), V18 (descriptions), V23/V24 (regressions), and the seal corroboration gate (denied over-stack keeps seal active with zero mutation; corroborated partial lifts it; never on prediction alone — the bridge's `partial_throw_execution_confirmed` predicate is the full DROP-identical corroboration). V16 Bug-B replay suppression proven via a bounded white-box fixture — CB emitted no natural replay in validation, so per the Stage 11 exit condition a constructed quarantine fixture drove the verbatim sanitizer + filter against real post-split ORS state, proving suppression of the true source-replay and non-overreach across successor/unrelated/wrong-container/temp_ref/promote entries, plus sanitizer rejection of tampered receipts. Bug A (issue #42, plan O8/O2) explicitly untouched, matching Stage 11's original scope. See companion roadmap Revision 7 for the full per-gate evidence. No architectural or contract change.
- **2026-07-22 — Revision 6 (Stage 8 complete; Stage 9 closed as an explicit split verdict).** Status updated: Stage 8 whole-THROW live block complete; Stage 9 recorded as a split verdict rather than a pass — TLS/ObjectHelper/ORS authority chain proven correct via the exact-count full-stack case GPT flagged as untested, but V17's whole-turn zero-duplication requirement fails on a plain single-action turn (new O8, distinct mechanism from O7: same-turn CB re-promotion after a correctly-deactivated seal, not an unsealed window). Filed as issue #42. User explicitly authorized continuing to Stages 10-11 on their original scope without expanding Stage 11 to contain this finding. See companion roadmap Revision 6 for the full per-row evidence. No architectural or contract change.
